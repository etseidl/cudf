...
#define RR 0
  ...
  while (s->error == 0 && processed_count < s->page.num_input_values) {
    ...
    while (pos < next_valid_count) {
      ...
      int const tid     = (t - t0 + plain_block_size) % plain_block_size;
      int const src_pos = pos;

      // the position in the output column/buffer
      int dst_pos = sb->nz_idx[rolling_index<plain_block_size * 2>(src_pos)] - s->first_row;
      if (dst_pos >= 0) {
        if (tid == 0) {
          auto offptr =
            reinterpret_cast<int32_t*>(nesting_info_base[leaf_level_index].data_out) + dst_pos;
...
#else
        for (size_type idx = tid; idx < len; idx += plain_block_size) {
          str_ptr[idx] = data[k + idx];
        }
        t0 = (t0 + len) % plain_block_size;
#endif
        string_offset += len;
      }
                                  ...
                                  kernel_error::pointer error_code)
{
  ...
  if (!setupLocalPageInfo(s,
                          ...
                          min_row,
                          num_rows,
                          mask_filter{decode_kernel_mask::STRING_FLAT_DICT},
                          page_processing_stage::DECODE)) {
    return;
  }
                             ...
                             rmm::cuda_stream_view stream)
{
  ...
  if (level_type_size == 1) {
    gpuDecodeStringPageDataFlatDict<uint8_t><<<dim_grid, dim_block, 0, stream.value()>>>(
      pages.device_ptr(), chunks, min_row, num_rows, error_code);
  } else {
    gpuDecodeStringPageDataFlatDict<uint16_t><<<dim_grid, dim_block, 0, stream.value()>>>(
      pages.device_ptr(), chunks, min_row, num_rows, error_code);
  }
}
